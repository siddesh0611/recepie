document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const userResponse = await axios.get('http://51.20.76.173:3000/user/profile', {
...
async function loadFavorites(collectionName) {
    const token = localStorage.getItem('token');
    try {
        const favoritesResponse = await axios.get(`http://51.20.76.173:3000/user/${collectionName}`, {
            headers: { "Authorization": token }
        });
