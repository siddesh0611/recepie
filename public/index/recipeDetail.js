...
async function getRecipeDetail(id) {
    try {
        const response = await axios.get(`http://13.60.56.141:3000/home/recipes/${id}`, {
...
async function getRecipeRating(recipeId) {
    try {
        const response = await axios.get(`http://13.60.56.141:3000/rate/rating/${recipeId}`, {
            headers: { "Authorization": token }
        });
...
async function rateRecipe(recipeId, userId, rating) {
    try {
        await axios.post('http://13.60.56.141:3000/rate/rate', { userId, recipeId, rating }, {
            headers: { "Authorization": token }
        });
...
async function followUser(userId) {
    try {
        console.log(userId);
        const response = await axios.post(`http://13.60.56.141:3000/home/follow/${userId}`, {}, {
            headers: { "Authorization": `${token}` }
        });
...
async function unfollowUser(userId) {
    try {
        const response = await axios.post(`http://13.60.56.141:3000/home/unfollow/${userId}`, {}, {
            headers: { "Authorization": `${token}` }
        });
        console.log(response.data.message);
...
async function saveToCollection() {
    ...
    try {
        await axios.post(`http://13.60.56.141:3000/user/save`, { recipeId, collection: selectedCollection }, {
            headers: { "Authorization": token }
        });
